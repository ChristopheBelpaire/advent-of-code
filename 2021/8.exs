input = "bg gcdaeb aebg efabdcg abdce cafdbe fcbdeg bdacg gbd cafgd | daecb dcbae gb eabg
...
defmodule AOC8 do
  def find_digits(input) do
    input = input
    |> String.split(" ", trim: true)
    |> Enum.map(&String.split(&1, "", trim: true))

    [fs1, fs2, fs3] = five_s = Enum.filter(input, fn(d) -> length(d) == 5 end)

    five_s =  if length(fs1 -- fs2 ) == 1, do: List.delete(five_s, fs3), else: five_s
    five_s =  if length(fs1 -- fs3 ) == 1, do: List.delete(five_s, fs2), else: five_s
    [three] =  if length(fs2 -- fs3 ) == 1, do: List.delete(five_s, fs1), else: five_s

    seven = Enum.find(input, fn(d) -> length(d) == 3 end)
    [ss1, ss2, ss3]  = Enum.filter(input, fn(d) -> length(d) == 6 end)
    six = nil
    six = if length(ss1 -- seven) == 4, do: ss1, else: six
    six = if length(ss2 -- seven) == 4, do: ss2, else: six
    six = if length(ss3 -- seven) == 4, do: ss3, else: six

    [fs1, fs2] = Enum.filter(input, fn(d) -> length(d) == 5 end) |> List.delete(three)

    [five, two] = if length(fs1 -- six) == 0, do: [fs1, fs2], else: [fs2, fs1]

    [ss1, ss2] = Enum.filter(input, fn(d) -> length(d) == 6 end) |> List.delete(six)

    four = Enum.find(input, fn(d) -> length(d) == 4 end)

    [zero, nine] = if length(ss1 -- four) == 3, do: [ss1, ss2], else: [ss2, ss1]


    digits = %{
      0 => zero,
      1 => Enum.find(input, fn(d) -> length(d) == 2 end),
      2 => two,
      3 => three,
      4 => four,
      5 => five,
      6 => six,
      7 => seven,
      8 => Enum.find(input, fn(d) -> length(d) == 7 end),
      9 => nine
    } |> Enum.map(fn({k, v}) -> {Enum.sort(v), k} end)
    |> Enum.into(%{})
  end

  def decode_result(input) do
    input
    |> String.split(" ", trim: true)
    |> Enum.map(fn(i) ->
      i
...
|> Enum.map(fn(l)->
  [d, r] = String.split(l, " | ")
  digits = AOC8.find_digits(d)
  AOC8.decode_result(r)
  |> Enum.map(fn(d) -> Map.get(digits, d)  |> Integer.to_string() end)
  |> Enum.join("")
  |> String.to_integer()
...
AOC8.find_digits("edbfga begcd cbg gc gcadebf fbgde acbgfd abcde gfcbed gfec")
