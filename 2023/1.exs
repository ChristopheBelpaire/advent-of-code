example = "twovgtprdzcjjzkq3ffsbcblnpq
...
|> Enum.reduce(0, fn(line, total) ->
  first = line |> String.split("") |> Enum.find(fn(d) -> Integer.parse(d)!= :error end)
  last = line |> String.split("")  |> Enum.reverse() |> Enum.find(fn(d) -> Integer.parse(d)!= :error end)
  total + String.to_integer("#{first}#{last}")
end)

pattern = :binary.compile_pattern(["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"])
...
|> String.split("\n")
|> Enum.reduce(0, fn(line, total) ->
  numbers = Regex.scan(~r/(?=(one|two|three|four|five|six|seven|eight|nine|1|2|3|4|5|6|7|8|9))/, line)
  ["", first] = hd(numbers)
  ["", last] = numbers |> Enum.reverse() |> hd()
  IO.inspect(numbers)
  first = Enum.find_index(digits, &(&1==first)) || first
  last = Enum.find_index(digits, &(&1==last)) || last
  total + String.to_integer("#{first}#{last}")
end)
