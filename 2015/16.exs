...
defmodule AOC16 do
  import NimbleParsec
  defparsec :parse_line, ignore(string("Sue "))
  |> integer(min: 1, max: 3)
  |> ignore(string(": "))
  |> repeat(
    ascii_string([?a..?z], min: 3, max: 15)
    |> ignore(string(": "))
    |> integer(min: 1, max: 3)
    |> ignore(string(", "))
  )
  ...
  def parse(owns, input) do
    input
    |> String.split("\n", trim: true)
    |> Enum.map(fn(line)->
      {:ok, line, _, _, _, _} = parse_line(line)
      [number | current] = line
      current = Enum.chunk_every(current, 2)
      if current -- owns == [], do: number, else: nil
    end)
    |> Enum.uniq()
  end
...
AOC16.parse(owns, "Sue 1: children: 1, cars: 8, vizslas: 7
...
Sue 500: cars: 1, perfumes: 6, vizslas: 1") |> IO.inspect()
