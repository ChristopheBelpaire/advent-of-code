defmodule AOC13 do
  ...
  def part1(list) do
    {list, people} = parse(list)
    people = ["Christophe" | people]
    people
    |> permutations()
    |> Enum.map(fn(permutation)->
      permutation = permutation ++ [hd(permutation)]
      pairs = Enum.chunk_every(permutation, 2, 1, :discard)
      (pairs ++ Enum.map(pairs, fn([a, b]) -> [b, a] end))
      |> Enum.map(&Map.get(list, &1, 0))
      |> Enum.sum()
    end)
    |> Enum.max()
  end
  ...
  def parse(list) do
    ...
    |> Enum.reduce(%{}, fn(line, acc) ->
       [from, _, sign, points, _, _, _ ,_ , _ , _, to] = String.split(line)
       points = if sign == "gain", do: String.to_integer(points), else: - String.to_integer(points)
       Map.put(acc, [from, String.trim(to, ".")], points)
    end)
    people = Enum.map(list, fn({[user, _],_})-> user end) |> Enum.uniq()
    {list, people}
  end
