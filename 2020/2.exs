...
defmodule Parser do
  ...
  def is_valid(l) do
    {:ok, [min, "-", max, " ", char, ": ", password], _, _, _, _} = Parser.line(l)
    freq = password
    |> String.split("", trim: true)
    |> Enum.count(&(&1==char))
    max >= freq and freq >= min
  end

  def is_valid_2(l) do
    {:ok, [p1, "-", p2, " ", char, ": ", password], _, _, _, _} = Parser.line(l)
    password = password
    |> String.split("", trim: true)
    (Enum.at(password, p1 - 1) == char and Enum.at(password, p2 - 1) != char) or
    (Enum.at(password, p2 - 1) == char and Enum.at(password, p1 - 1) != char)
  end
end


input = "1-7 j: vrfjljjwbsv
1-10 j: jjjjjjjjjjjj
...
|> String.split("\n")
|> Enum.count(fn(l) ->
  Parser.is_valid_2(l) |> IO.inspect()
end)
|> IO.inspect()
